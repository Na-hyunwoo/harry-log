...
Cookie는 사용자가 어떤 웹사이트에 방문할 경우 서버에 의해 사용자의 브라우저에 **저장되는 작은 데이터입니다.**

그러면 브라우저에 저장되는 작은 데이터가 어떻게 번거로웠던 인가 절차를 축소시킬 수 있었을까요 ?

![cookie-process](https://velog.velcdn.com/images/dusdjeks/post/a4d225dd-5660-4c50-a322-c14d70eb4c25/image.png)

서버가 HTTP 응답 헤더의 Set-Cookie에 내용을 넣어 전달하면, 브라우저는 이 내용을 자체적으로 브라우저에 저장합니다. **브라우저는 사용자가 쿠키를 생성하도록 한 동일 사이트에 접속할 때마다 쿠키의 내용을 Cookie 요청 헤더에 넣어서 함께 전달합니다.** 따라서, 인증된 클라이언트임을 알 수 있는 것이지요 !
...
이 방식은 Cookie에 ID/PW와 같은 중요 정보들을 담는 것이 아니라, **인증을 위한 별개의 정보를 서버의 세션 저장소에 저장하는 방식**입니다. 클라이언트는 이 정보를 Cookie에 유니크한 키 형태로 담아서 서버에 요청합니다. 이 때, 서버는 세션 저장소에 있는 정보와 요청한 정보가 일치하는지 확인합니다.
...
![jwt-process](https://velog.velcdn.com/images/dusdjeks/post/79dc1bfe-b54f-4532-a205-c3fc43ddf792/image.png)

### JWT 구조

![jwt-architecture](https://velog.velcdn.com/images/dusdjeks/post/aa5fca4c-991c-441b-916b-5532295ea9dd/image.png)
